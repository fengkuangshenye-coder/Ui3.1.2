...
const RATE_RST_PER_USD = 500; // 示例汇率

export default function PrivateSalePanel({ onToast }: PrivateSalePanelProps) {
      ...
      <CardContent className="grid gap-4 text-sm">
        <div className="flex items-center gap-2">
          ...
          <span className="text-muted-foreground ml-auto">
            时间(UTC)：{CFG.privateSale.timeUTC}
          </span>
        </div>
        ...
        <div className="grid gap-2">
          ...
          <p className="text-xs text-muted-foreground">
            预计可得：
            <span className="text-cyan-300 font-medium">{rst} RST</span>
            <span className="ml-2">
              {asset === "BNB"
                ? `按 BNB ≈ $${bnbPrice.toFixed(2)} 折算`
                : `按 USDT ≈ $1.00 折算`}
            </span>
          </p>
        </div>
          ...
          {[
            ...
            <div
              ...
              className={`rounded-lg px-3 py-2 border border-white/10 bg-black/30 flex items-center justify-center ${
                step >= s.k
                  ? "ring-1 ring-cyan-500/50 shadow-[0_0_24px_#22d3ee40]"
                  : ""
              }`}
            >
              {s.t}
